...
def image_servingmodel(dataset,
    ...
    print(dataset, ip, port, model_name, label_map_path, source)
    ...
    def get_image_stream(stream, thresh=0.5):
        for eg in stream:
            ...
            predictions = get_predictions(np_image, ip, port, model_name)
    ...
    def get_predictions(numpy_image, ip, port, model_name):
        boxes, class_ids, scores = _tf_odapi_client(np.expand_dims(numpy_image,
                                                                   axis=0),
                                                    ip,
                                                    port, model_name
                                                    )
...
def _generic_tf_serving_client(data, ip, port, model_name,
    ...
    request.inputs['{}'.format(input_name)
                   ].CopyFrom(tf.contrib.util.make_tensor_proto(data,
                                                                shape=data.shape
                                                                ))
